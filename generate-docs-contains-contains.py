                ...
                if link.get('ltype'):
                    if link['ltype'] == 'Contains':
                        ...
                        if not links_map.get(link_id_1) and not links_map.get(link_id_2):
                            links_map[link_id_1] = {'source': doc_node, 'target': link_node, 'count': 1, 'type': link['ltype']}
...
with open('docs/visuals/docs-graph-contains.dot', 'w') as html_file:
    template = open('templates/visuals/graphviz.dot').read();
    html_file.write(template
                    .replace('${nodes}', '')
                    .replace('${links}', links_text))

with open('docs/visuals/force-graph-3d-contains.html', 'w') as html_file:
